...
PLATFORMS = ["aihuishou", "xianyu", "pinduoduo"]
    ...
    def fetch_price(self, product: dict) -> PriceRecord:
        ...
        try:
            ...
            return PriceRecord(
                ...
                source_url=url, crawl_time=self._now(), status="success"
            )
...
class PinduoduoCrawler(BaseCrawler):
    ...
    def fetch_price(self, product: dict) -> PriceRecord:
        ...
        try:
            ...
            if self.session is None:
                raise ImportError("requests not installed")
            resp = self.session.get(url, timeout=15)
            price = self._parse_lowest(resp.text)
            return PriceRecord(
                product_id=product["id"], product_name=product["name"],
                platform=self.platform, price=price, condition="全新最低",
                source_url=url, crawl_time=self._now(), status="success"
            )
        except Exception as e:
            return PriceRecord(
    ...
    def _parse_lowest(self, html: str) -> Optional[float]:
        prices = [float(p) for p in re.findall(r'(\d{2,5}\.?\d{0,2})', html)]
    ...
    def _save_price_cache(self, records: List[dict]):
        ...
        for record in records:
            ...
            if platform == "aihuishou":
                ...
                product["latest_prices"]["new_price"] = record["price"]
